...
import com.glygateway.service.triton.config.Gemma3Config;
...
public class Gemma3Adapter extends AbstractModelAdapter {
  ...
  public Gemma3Adapter(ChatTemplateRegistry chatTemplateRegistry,
  ...
  @Override
  public Mono<String> debugApplyChatTemplate(Conversation agentChatRequest) throws ValidationException {
    return Mono.fromCallable(() -> chatTemplate.applyTemplate(new Conversation(agentChatRequest.getContents())));
  }

  @Override
  public Mono<ModelInferRequest> buildRequest(Conversation agentChatRequest, InferenceParams inferenceParams)
      throws ValidationException {
    return Mono.fromCallable(() -> {
      ...
      float float_temperature = inferenceParams.getTemperature() != null ? inferenceParams.getTemperature()
          : config.temperature();
      ...
      Optional<ByteString> input_stop_tokens_byte = stopTokens.stream().map(BufferCodec::encodeStringToBytes)
          .reduce((a, b) -> a.concat(b));

      var bool_exclude_input_in_output = inferenceParams.getExcludeInputInOutput() != null
          ? inferenceParams.getExcludeInputInOutput()
          : true;
      ...
      return ModelInferRequest.newBuilder()
          ...
          .addRawInputContents(
              BufferCodec
                  .encodeStringToBytes(chatTemplate.applyTemplate(new Conversation(agentChatRequest.getContents()))))
