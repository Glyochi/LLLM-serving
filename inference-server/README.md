...
- [Conceptual guide](https://github.com/triton-inference-server/tutorials/tree/main/Conceptual_Guide)
    ...
    - [Part 3](https://github.com/triton-inference-server/tutorials/tree/main/Conceptual_Guide/Part_3-optimizing_triton_configuration)
        - [Model analyzer](https://github.com/triton-inference-server/model_analyzer/blob/main/docs/install.md#specific-version-with-local-launch-mode)
            ...
            - Use [Yaml Config](https://github.com/triton-inference-server/model_analyzer/blob/main/docs/config.md#table-of-contents) now instead of flags
            - [genai-perf and model_analyzer version mismatch](https://github.com/triton-inference-server/model_analyzer/issues/935)
                ...
                - Customize through `genai_perf_flags`  
                - It will looks for the `tokenizer` for the benchmarking model, so specify `tokenizer: local_path/.../tokenizer` 
            ...
            - `model_analyzer_model_repository`
                ...
                - `model_analyzer` have to be able to access that path `A_path`, and reuse that `A_path` to spin up `B` and specify mounting `local_path` to `B_path` 
                - And `tritonserver` in `B` has to able to know where `B_path` is, from the command line generated by `model_analyzer` in `A`
                => TLDR: `local_path` and `container path` should be the same, and both be `absolute path`
                => We won't be reusing `model-repository` folder, but make a new one called `model_analyzer_model_repository`
                => `fill_config.sh` script is reusable LEGGGO to update `local` and `container` path to be the same
            - Spun up container to do inferencing by `model_analyzer` has a few fields that seem to cannot be overwritten
                - `--model-control-mode` has to be `explicit` mode, and `--strict-model-config` cannot be change, default to `true`, not sure why tho
                - because of this, have to add `load_model: "*"` or `--load-model=*` flag to the command line  
                    - Or just specifying all the models one by one, but I havent been able to get that figured out
